...
**启动诊断包（URL / token 指纹 / 端口）**

- 在启动 **Relay** 或 **Bridge** 前设置：`OPENCLAW_DIAG=1`（PowerShell：`$env:OPENCLAW_DIAG="1"`；也可用 `DEVICE_BRIDGE_DIAG=1`）。
- 进程会在日志里打印带边框的「诊断包」，并打开 DEBUG（含 `websockets` 等）。用 **`token_fp`**（密钥的 SHA256 固定前缀，不含口令长度）对照两端是否一致；详见 `shared/diagnostics.py`。
